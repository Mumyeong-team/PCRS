...
| 📌 프로젝트명   | FashionPeople           |
| ---------- | ----------------------- |
| 🎯 핵심 목표   | 신체 데이터 기반 의류 추천 및 3D 피팅 |
| 🧠 분석 방식   | OpenCV + MediaPipe Pose |
| 🧩 데이터 형식  | JSON 기반 체형 분석 데이터       |
| 🖥️ 서비스 형태 | Web 기반 개인 맞춤형 패션 분석 서비스 |
...
**FashionPeople**은 사용자가 업로드한 **정면·측면 이미지**를 분석하여 신체 좌표를 추출하고, 체형 데이터를 기반으로 **AI 스타일 추천**과 **3D 아바타 피팅**을 제공하는 웹 기반 개인 맞춤형 패션 분석 서비스입니다.

온라인 의류 쇼핑에서는 브랜드마다 사이즈 기준이 다르고, 사용자는 실제로 옷을 입어보기 전까지 자신에게 맞는 핏을 정확히 예측하기 어렵습니다.
특히 같은 사이즈의 의류라도 사람마다 **어깨 너비, 허리 비율, 상체와 하체 길이, 팔 길이** 등이 다르기 때문에 실제 착용감은 크게 달라질 수 있습니다.

FashionPeople은 이러한 문제를 해결하기 위해 사용자의 이미지에서 직접 신체 좌표를 추출하고, 이를 체형 분석 데이터로 변환하여 **추천 시스템과 3D 피팅 기능에 연결**합니다.
...
| 리뷰 중심          | 체형 비율 기반      |
| 구매 이력 중심       | AI 스타일 추천     |
| 사용자가 직접 판단     | 분석 결과 기반 추천   |
| 실제 착용 전 확인 어려움 | 3D 아바타 기반 피팅  |
...
| 기능                      | 설명                        |
| ----------------------- | ------------------------- |
| 📸 이미지 기반 체형 분석         | 정면·측면 이미지를 업로드하여 신체 좌표 추출 |
| 🧍 MediaPipe Pose 좌표 추출 | 33개 신체 랜드마크 좌표 생성         |
| 📊 체형 데이터 생성            | 어깨, 허리, 골반, 상하체 비율 계산     |
| 🧠 AI 스타일 추천            | 체형 분류 결과 기반 스타일 추천        |
| 🧑‍💻 3D 아바타 피팅         | 체형 데이터를 3D 아바타 변형에 활용     |
| 🗄️ DB 저장 구조            | 분석 결과, 추천 결과, 피팅 결과 저장    |
...
MediaPipe Pose는 사람의 신체를 총 **33개의 랜드마크 좌표**로 인식할 수 있습니다.
...
| 영역                | 역할                                    |
| ----------------- | ------------------------------------- |
| Frontend          | 이미지 업로드, 결과 화면, 3D 아바타 출력             |
| Backend Server    | 이미지 수신, 체형 분석, JSON 생성, API 제공        |
| AI Recommendation | 체형 분류 결과 기반 스타일 추천                    |
| 3D Avatar         | Blender / Mixamo / Three.js 기반 아바타 피팅 |
| Database          | 세션, 체형 분석 결과, 의류 정보, 추천 결과 저장         |
...
| 구분                  | 기술                                                          |
| ------------------- | ----------------------------------------------------------- |
| Frontend            | React, TypeScript, Three.js, HTML, CSS                      |
| Backend             | Python, FastAPI, OpenCV, MediaPipe, Pydantic                |
| AI / Recommendation | MediaPipe Pose, Rule-based Body Type Classification, AI API |
| 3D                  | Blender, Mixamo, Three.js, GLB, Shape Key, Morph Target     |
...
| 테이블명                    | 역할                |
| ----------------------- | ----------------- |
| `guest_session`         | 사용자 세션 정보 저장      |
| `body_analysis`         | 신체 분석 결과 저장       |
| `clothing_library`      | 의류 정보 및 모델 경로 관리  |
| `recommendation_result` | AI 스타일 추천 결과 저장   |
| `fitting_result`        | 3D 피팅 및 아바타 결과 저장 |
...
| AI 추천 요청     | GET / POST | `/recommend`            | 체형 분석 결과 기반 스타일 추천 생성       |
...
<p align="center">
  <b>FashionPeople은 사용자의 신체 데이터를 기반으로 체형 분석, AI 스타일 추천, 3D 아바타 피팅을 연결하는 개인 맞춤형 패션 분석 서비스입니다.</b>
</p>
