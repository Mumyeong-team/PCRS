...
서버는 업로드된 이미지를 다음과 같은 형태로 저장합니다.
...
이후 OpenCV와 MediaPipe Pose를 활용하여 신체 관절 좌표를 추출합니다.
...
```json
{
  "shoulder_width": 0.25875,
  "waist_width": 0.13454,
  "hip_width": 0.20123,
  "arm_length": 0.31245,
  "upper_body_length": 0.24567,
  "lower_body_length": 0.42189,
  "upper_lower_ratio": 0.58,
  "shoulder_waist_ratio": 1.92,
  "body_type": "상체형"
}
```

위 값들은 실제 cm 단위가 아니라, 이미지 크기 기준으로 정규화된 신체 비율 데이터입니다.
...
```bash
git clone https://github.com/Mumyeong-team/PCRS.git
cd PCRS
```
...
정면 이미지와 측면 이미지를 업로드하고, 체형 분석을 수행합니다.
...
1. 서버 실행
2. 이미지 업로드
3. MediaPipe 기반 좌표 추출
4. front.json / side.json / body_result.json 생성
5. 체형 분석 결과 확인
6. AI 추천 결과 확인
7. 3D 아바타 연동 결과 확인
```
